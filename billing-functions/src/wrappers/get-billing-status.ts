...
export default async function getBillingStatus(
  ...
  const data = await upsertCustomerSubscription(supabaseClient, roleInfo.account_id, billingData);
  ...
  return {
    ...
    subscription_active: ["trialing", "active"].includes(billingData?.subscription?.status),
